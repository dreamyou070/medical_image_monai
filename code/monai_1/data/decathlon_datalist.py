...
def _append_paths(base_dir: PathLike, is_segmentation: bool, items: list[dict]) -> list[dict]:
    ...
    Args:
        ...
        items: list of data_module items, each of which is a dict keyed by element names.
...
def load_decathlon_datalist(
    ...
    Raises:
        ...
        ValueError: When ``data_list_key`` is not specified in the data_module list file.

    Returns a list of data_module items, each of which is a dict keyed by element names, for example:

    .. code-block::

        [
            {'image': '/workspace/data_module/chest_19.nii.gz',  'label': 0},
            {'image': '/workspace/data_module/chest_31.nii.gz',  'label': 1}
        ]
...
def load_decathlon_properties(data_property_file_path: PathLike, property_keys: Sequence[str] | str) -> dict:
    """Load the properties from the JSON file contains data_module property with specified `property_keys`.

    Args:
        data_property_file_path: the path to the JSON file of data_module properties.
    ...
    for key in ensure_tuple(property_keys):
        if key not in json_data:
            raise KeyError(f"key {key} is not in the data_module property file.")
...
def check_missing_files(
    ...
    Args:
        datalist: a list of data_module items, every item is a dictionary.
