...
DUMMY_MODULES_FOLDER = "diffuser_module.utils"
...
def is_safetensors_compatible(filenames, variant=None, passed_components=None) -> bool:
    ...
    - For models from the diffuser_module library, just replace the ".bin" extension with ".safetensors"
...
def warn_deprecated_model_variant(pretrained_model_name_or_path, use_auth_token, variant, revision, model_filenames):
    ...
    if set(comp_model_filenames) == set(model_filenames):
        warnings.warn(
            f"You are loading the variant {revision} from {pretrained_model_name_or_path} via `revision='{revision}'` even though you can load it via `variant=`{revision}`. Loading model variants via `revision='{revision}'` is deprecated and will be removed in diffuser_module v1. Please use `variant='{revision}'` instead.",
            FutureWarning,
        )
    else:
        warnings.warn(
            f"You are loading the variant {revision} from {pretrained_model_name_or_path} via `revision='{revision}'`. This behavior is deprecated and will be removed in diffuser_module v1. One should use `variant='{revision}'` instead. However, it appears that {pretrained_model_name_or_path} currently does not have the required variant filenames in the 'main' branch. \n The Diffusers team and community would be very grateful if you could open an issue: https://github.com/huggingface/diffusers/issues/new with the title '{pretrained_model_name_or_path} is missing {revision} files' so that the correct variant file can be added.",
            FutureWarning,
        )
...
def load_sub_model(
    ...
    # When loading a transformers model, if the device_map is None, the weights will be initialized as opposed to diffuser_module.
...
class DiffusionPipeline(ConfigMixin, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        ```py
        >>> from diffuser_module import DiffusionPipeline
        ...
        >>> from diffuser_module import LMSDiscreteScheduler
        ...
        if pipeline_class.__name__ == "StableDiffusionInpaintPipeline" and version.parse(
            ...
            from diffuser_module import StableDiffusionInpaintPipeline, StableDiffusionInpaintPipelineLegacy
        ...
        # import it here to avoid circular import
        from diffuser_module import pipelines
    ...
    def download(cls, pretrained_model_name, **kwargs) -> Union[str, os.PathLike]:
        ...
        Parameters:
            ...
            use_auth_token (`str` or *bool*, *optional*):
                The token to use as HTTP bearer authorization for remote files. If `True`, the token generated from
                `diffuser_module-cli login` (stored in `~/.huggingface`) is used.
            revision (`str`, *optional*, defaults to `"main"`):
                The specific model version to use. It can be a branch name, a tag name, a commit id, or any identifier
                allowed by Git.
            custom_revision (`str`, *optional*, defaults to `"main"`):
        ...
        if not local_files_only:
            ...
            for component in folder_names:
                ...
                if candidate_file in filenames:
                    ...
                    raise ValueError(
                        f"{candidate_file} as defined in `model_index.json` does not exist in {pretrained_model_name} and is not a module in 'diffuser_module/pipelines'."
                    )
    ...
    def enable_attention_slicing(self, slice_size: Optional[Union[str, int]] = "auto"):
        ...
        ```py
        >>> import torch
        >>> from diffuser_module import StableDiffusionPipeline
