...
def convert_state_dict_to_peft(state_dict, original_type=None, **kwargs):
    r"""
    Converts a state dict to the PEFT format The state dict can be from previous diffuser_module format (`OLD_DIFFUSERS`), or
    new diffuser_module format (`DIFFUSERS`). The method only supports the conversion from diffuser_module old/new to PEFT for now.
...
def convert_state_dict_to_diffusers(state_dict, original_type=None, **kwargs):
    r"""
    Converts a state dict to new diffuser_module format. The state dict can be from previous diffuser_module format
    (`OLD_DIFFUSERS`), or PEFT format (`PEFT`) or new diffuser_module format (`DIFFUSERS`). In the last case the method will
    return the state dict as is.

    The method only supports the conversion from diffuser_module old, PEFT to diffuser_module new for now.
...
def convert_unet_state_dict_to_peft(state_dict):
    r"""
    Converts a state dict from UNet format to diffuser_module format - i.e. by removing some keys
    """
