...
class SimplexNoise(object):
    ...
    def __init__(self,
        ...
        if DIMENSIONS_2D == dimensions:
            self.octaves = [SimplexNoiseOctave2D() for i in range(self.num_octaves)]
            self.noise_scale = DEFAULT_2D_NOISE_SCALE

        elif DIMENSIONS_3D == dimensions:
            self.octaves = [SimplexNoiseOctave3D() for i in range(self.num_octaves)]
            self.noise_scale = DEFAULT_2D_NOISE_SCALE
        ...
        self.amplitudes = [pow(persistence, len(self.octaves) - i) for i in range(self.num_octaves)]

    def noise(self, x=0, y=0, z=0):
        noise = [self.octaves[i].noise( xin=x / self.frequencies[i],
                                        yin=y / self.frequencies[i],
                                        zin=z / self.frequencies[i],
                                        noise_scale=self.noise_scale) * self.amplitudes[i] for i in range(self.num_octaves)]
        return sum(noise)
...
class SimplexNoiseOctave3D(object):
    ...
    def __init__(self, num_shuffles=DEFAULT_SHUFFLES):
        self.p_supply = [i for i in range(0, 256)]
        self.grads = [Point(1, 1, 0), Point(-1, 1, 0), Point(1, -1, 0), Point(-1, -1, 0),
...
def main() :
    simplex_instance = SimplexNoise(num_octaves=6,
                                    persistence=0.8,
                                    dimensions=3)
