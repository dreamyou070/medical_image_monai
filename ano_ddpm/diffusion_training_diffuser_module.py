                ...
                with torch.no_grad():
                    for t in range(args.sample_distance, -1, -1):
                        if t > 0:
                            ...
                            pred_x_0 = diffusion.step(model_output,t,x_t,)['pred_original_sample']
                ...
                if epoch % args.inference_freq == 0 and step == 0:
                    for i, test_data in enumerate(test_dataset_loader):
                        if i == 0:
                            ...
                            training_outputs(scheduler, test_data, epoch, inference_num, save_imgs=args.save_imgs,
                                             ema=ema, args=args, is_train_data = False, device = device)
                            training_outputs(scheduler, data, epoch, inference_num, save_imgs=args.save_imgs,
