...
class GaussianDiffusionModel:
    ...
    def q_sample(self, x_0, t, noise, denoise_fn):
        """
            q (x_t_1 | x_t, x_0 )
            :param t: torh tensor of shape (batch_size, 1)
            :param noise:
            :return:
        """
        x_t = self.sample_q(x_0, t, noise)
        posterior_mean, _, posterior_log_var = self.q_posterior_mean_variance(x_0, x_t, t)
        if type(denoise_fn) == str:
            if denoise_fn == "gauss":
                noise = torch.randn_like(x_t)
            elif denoise_fn == "noise_fn":
            ...
            noise = denoise_fn(x_t, t)
        nonzero_mask = ((t != 0).float().view(-1, *([1] * (len(x_t.shape) - 1))))
        sample = posterior_mean + nonzero_mask * torch.exp(0.5 * posterior_log_var) * noise
        return {"sample": sample,}
